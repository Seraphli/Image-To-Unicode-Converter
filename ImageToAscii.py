...
def map_pixels_to_ascii_chars(image, range_width=255 / (len(ASCII_CHARS) - 1), reverse=False):
    ...
    pixels_to_chars = []
    for pixel_value, alpha in pixels_in_image:
        if alpha == 0:
            pixel_value = 255
        if reverse:
...
def convert_image_to_ascii(image, new_width=100, cov=1, bv=1, cv=1, sv=1, reverse=False):
    image = enhance_image(image, cov, bv, cv, sv)
    image = scale_image(image, int(new_width))
    image = convert_to_grayscale(image)

    pixels_to_chars = map_pixels_to_ascii_chars(image, reverse=reverse)
...
def handle_image_conversion(image_filepath, new_width, cov, bv, cv, sv):
    ...
    with codecs.open("temp.txt", 'w', "utf-8") as file1:
        file1.write(image_ascii)

    image_ascii = convert_image_to_ascii(image, new_width, cov, bv, cv, sv, reverse=True)
    with codecs.open("temp_1.txt", 'w', "utf-8") as file1:
        file1.write(image_ascii)
